...
class Compiler
  ...
  def compile(keymap_data, lhs_kconfig_data, rhs_kconfig_data)
    if rhs_kconfig_data && !rhs_kconfig_data.empty?
      lhs_result, lhs_output = compile_board('glove80_lh', keymap_data, lhs_kconfig_data, include_static_rhs: false)
      rhs_result, rhs_output = compile_board('glove80_rh', keymap_data, rhs_kconfig_data, include_static_rhs: false)
      [
        lhs_result.concat(rhs_result),
        ["LHS Output:", *lhs_output, "RHS Output:", *rhs_output],
      ]
    else
      compile_board('glove80_lh', keymap_data, lhs_kconfig_data, include_static_rhs: true)
    end
  end
  ...
  def compile_board(board, keymap_data, kconfig_data, include_static_rhs: false)
    in_build_dir do
      ...
      compile_command = ['compileZmk', '-b', board, '-k', './build.keymap']
